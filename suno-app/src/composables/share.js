...
const mac = navigator.userAgent.match(/iPhone|iPad|iPod|Macintosh/i);
const isDesktop = !(ios || android);

export const polyfilledShare = function () {
  const languages = {
    default: {
      sms: "SMS",
      messenger: "Messenger",
...
<button class="${!configs.line ? "hidden" : ""} tool-icon line" data-tool="line">
 ${icon.line}
 <span class="the-icon-title">${language.line}</span>
</button>
</div>
...
`;
      ...
      function bindEvents() {
        Array.from(container.querySelectorAll(".tool-icon")).forEach((tool) => {
          tool.addEventListener("click", (event) => {
            ...
            switch (tool.dataset.tool) {
              ...
              case "linkedin": {
                window.open(
                  `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(url)}&title=${title}&summary=${text}&source=LinkedIn`,
                );
                break;
              }
              case "telegram": {
        ...
        container
          .querySelector(".shareAPIPolyfill-footer")
          .addEventListener("click", closeShareUI);
      }
    });
  };
};
...
navigator.share = isMobile()
  ? navigator.share || polyfilledShare()
  : polyfilledShare();
